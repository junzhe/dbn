...
groundTruth = full(sparse(labels, 1:numClasses, 1));  %numClasses*M
cost = 0;
...
cost = -1/numClasses*sum(sum(groundTruth.*log(h)))+lambda/2*sum(sum(theta.^2));
thetagrad = -1/numClasses*((groundTruth-h)*data')+lambda*theta;%log(h)
